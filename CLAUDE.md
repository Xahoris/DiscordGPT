...
- use pydantic and langchain if necessary
- this project is a very simple MVP. Keep files to a minimum
...
The project must maintain a lightweight but reliable change tracking system.

### CHECKPOINT.md

- Ensure a `CHECKPOINT.md` file exists at the root of the repository.
- Update `CHECKPOINT.md` **whenever meaningful changes are made** to the project.
- Use a clear, structured format for entries (date, scope, short description).

### Change Summaries

- Each update must include a **concise summary** of what was changed.
- Summaries should be written so that a developer can quickly understand the current project state when reopening it after a break.
- Avoid verbose or low-value entries; focus on architectural, functional, or behavioral changes.
